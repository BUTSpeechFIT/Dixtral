...
class DataCollator:
    ...
    def __call__(self, inputs: List[Dict[str, Union[List[int], torch.Tensor]]], nested=False) -> BatchFeature:
        ...
        feats = pad_sequence([sample['input_features'] for sample in inputs], batch_first=True).transpose(1,-1)
        masks = pad_sequence([sample['attention_mask'] for sample in inputs], batch_first=True)

        stno_masks = pad_sequence([sample['stno_mask'].T for sample in inputs], batch_first=True)
